(function () {
  "use strict";

  function g(a) {
    console.log("LoaderBootstrap: [INFO] " + a);
  }

  function n(a) {
    console.log("LoaderBootstrap: [WARN] " + a);
  }

  function q(a) {
    console.error("LoaderBootstrap: [ERROR] " + a);
  }

  var r = null;

  // Track blob URLs for post-launch cleanup
  window.__eaglercraftXBlobURLs = [];
  function createTrackedBlobURL(blob) {
    var url = URL.createObjectURL(blob);
    window.__eaglercraftXBlobURLs.push(url);
    return url;
  }

  function u() {
    const a = [];
    for (var c = 0; 64 > c; ++c)
      a[
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charCodeAt(
          c,
        )
      ] = c;
    a[45] = 62;
    a[95] = 63;

    return function (b, d) {
      var e = b.length - d;
      if (0 < e % 4)
        throw Error("Invalid string. Length must be a multiple of 4");
      var f = b.indexOf("=", d);
      f = -1 === f ? e : f - d;
      e = [f, f === e ? 0 : 4 - (f % 4)];
      var l = e[0];
      e = e[1];
      f = new Uint8Array((3 * (l + e)) / 4 - e);
      var h = 0;
      l = (0 < e ? l - 4 : l) + d;
      var k;
      for (k = d; k < l; k += 4) {
        d =
          (a[b.charCodeAt(k)] << 18) |
          (a[b.charCodeAt(k + 1)] << 12) |
          (a[b.charCodeAt(k + 2)] << 6) |
          a[b.charCodeAt(k + 3)];
        f[h++] = (d >> 16) & 255;
        f[h++] = (d >> 8) & 255;
        f[h++] = d & 255;
      }
      if (2 === e) {
        d = (a[b.charCodeAt(k)] << 2) | (a[b.charCodeAt(k + 1)] >> 4);
        f[h++] = d & 255;
      } else if (1 === e) {
        d =
          (a[b.charCodeAt(k)] << 10) |
          (a[b.charCodeAt(k + 1)] << 4) |
          (a[b.charCodeAt(k + 2)] >> 2);
        f[h++] = (d >> 8) & 255;
        f[h++] = d & 255;
      }
      return f.buffer;
    };
  }

  function v() {
    return new Promise(function (a) {
      setTimeout(a, 20);
    });
  }

  function w(a) {
    return new Promise(function (c) {
      const b = new Image();
      b.addEventListener("load", c);
      b.addEventListener("error", function () {
        n("Failed to preload image: " + a);
        c();
      });
      b.src = a;
      setTimeout(c, 50);
    });
  }

  function x(a) {
    return new Promise(function (c) {
      fetch(a, { cache: "force-cache" })
        .then(function (b) {
          return b.arrayBuffer();
        })
        .then(c)
        .catch(function (b) {
          q("Failed to fetch URL! " + b);
          c(null);
        });
    });
  }

  function y(a) {
    return a.startsWith("data:application/octet-stream;base64,")
      ? new Promise(function (c) {
          x(a).then(function (b) {
            if (b) c(b);
            else {
              n(
                "Failed to decode base64 via fetch, doing it the slow way instead...",
              );
              try {
                r ||= u();
                var d = r(a, 37);
                c(d);
              } catch (e) {
                q("Failed to decode base64! " + e);
                c(null);
              }
            }
          });
        })
      : x(a);
  }

  function z(a, c) {
    const b = document.createElement("h2");
    b.style.color = "#AA0000";
    b.style.padding = "25px";
    b.style.fontFamily = "sans-serif";
    b.style.marginBlock = "0px";
    b.appendChild(document.createTextNode(c));
    a.appendChild(b);

    const h4 = document.createElement("h4");
    h4.style.color = "#AA0000";
    h4.style.padding = "25px";
    h4.style.fontFamily = "sans-serif";
    h4.style.marginBlock = "0px";
    h4.appendChild(document.createTextNode("Try again later"));
    a.style.backgroundColor = "white";
    a.appendChild(h4);
  }

  function parseParams() {}

  window.main = async function () {
    if ("undefined" === typeof window.eaglercraftXOpts) {
      q("window.eaglercraftXOpts is not defined!");
      alert("window.eaglercraftXOpts is not defined!");
      return;
    }

    var a = window.eaglercraftXOpts.container;
    if ("string" !== typeof a) {
      q("window.eaglercraftXOpts.container is not a string!");
      alert("window.eaglercraftXOpts.container is not a string!");
      return;
    }

    var c = window.eaglercraftXOpts.assetsURI;
    if ("string" !== typeof c) {
      if (
        "object" === typeof c &&
        "object" === typeof c[0] &&
        "string" === typeof c[0].url
      ) {
        c = c[0].url;
      } else {
        q("window.eaglercraftXOpts.assetsURI is not a string!");
        alert("window.eaglercraftXOpts.assetsURI is not a string!");
        return;
      }
    }

    if (c.startsWith("data:")) {
      delete window.eaglercraftXOpts.assetsURI;
    }

    var b = document.getElementById(a);
    if (!b) {
      const err =
        'window.eaglercraftXOpts.container "' +
        a +
        '" is not a known element id!';
      q(err);
      alert(err);
      return;
    }

    while ((a = b.lastChild)) b.removeChild(a);

    a = document.createElement("div");
    a.style.width = "100%";
    a.style.height = "100%";
    a.style.setProperty("image-rendering", "pixelated");
    a.style.background =
      'center / contain no-repeat url("data:image/png;base64,") white';
    b.appendChild(a);

    await v();

    if (c.startsWith("data:")) {
      g('Downloading EPW file "<data: ' + c.length + ' chars>"...');
      c = await y(c);
    } else {
      g('Downloading EPW file "' + c + '"...');
      c = await x(c);
    }

    var d = !1;
    if (!c || 384 > c.byteLength) d = !0;

    if (d) {
      b.removeChild(a);
      z(b, "Failed to download EPW file!");
      q("Failed to download EPW file!");
      return;
    }

    var e = new DataView(c);
    if (608649541 !== e.getUint32(0, !0) || 1297301847 !== e.getUint32(4, !0))
      d = !0;
    if (e.getUint32(8, !0) !== c.byteLength) d = !0;

    if (d) {
      b.removeChild(a);
      z(b, "EPW file is invalid!");
      q("EPW file is invalid!");
      return;
    }

    var l = new TextDecoder("utf-8"),
      h = e.getUint32(100, !0),
      k = e.getUint32(104, !0),
      m = e.getUint32(108, !0),
      p = e.getUint32(112, !0);

    if (0 > h || h + k > c.byteLength || 0 > m || m + p > c.byteLength) d = !0;

    if (d) {
      b.removeChild(a);
      z(b, "EPW file is invalid!");
      q("EPW file is invalid!");
      return;
    }

    h = new Uint8Array(c, h, k);
    m = new Uint8Array(c, m, p);
    l = createTrackedBlobURL(new Blob([h], { type: l.decode(m) }));

    await w(l);
    g("Loaded splash img: " + l);

    a.style.background =
      'center / contain no-repeat url("' +
      l +
      '"), 0px 0px / 1000000% 1000000% no-repeat url("' +
      l +
      '") white';

    await v();

    p = e.getUint32(164, !0);
    h = e.getUint32(168, !0);
    m = e.getUint32(180, !0);
    e = e.getUint32(184, !0);

    if (0 > p || p + h > c.byteLength || 0 > m || m + e > c.byteLength) d = !0;

    if (d) {
      b.removeChild(a);
      z(b, "EPW file is invalid!");
      q("EPW file is invalid!");
      return;
    }

    a = new Uint8Array(c, p, h);
    a = createTrackedBlobURL(
      new Blob([a], { type: "text/javascript;charset=utf-8" }),
    );

    g("Loaded loader.js: " + a);

    d = new Uint8Array(c, m, e);
    d = createTrackedBlobURL(new Blob([d], { type: "application/wasm" }));

    g("Loaded loader.wasm: " + d);

    var f = {};
    for (const [t, A] of Object.entries(window.eaglercraftXOpts)) {
      if ("container" !== t && "assetsURI" !== t) f[t] = A;
    }

    window.__eaglercraftXLoaderContextPre = {
      rootElement: b,
      eaglercraftXOpts: f,
      theEPWFileBuffer: c,
      loaderWASMURL: d,
      splashURL: l,
    };

    g("Appending loader.js to document...");

    b = document.createElement("script");
    b.type = "text/javascript";
    b.src = a;
    document.head.appendChild(b);
  };

  window.parseParams = parseParams;
}).call(this);
